...
void player::move(int dx, int dy) {
    ...
    if (cell == FLOOR || cell == GOAL) {
        ...
        if (target_cell == FLOOR || target_cell == GOAL) {
            ...
            for (size_t level_row = 0; level_solved && level_row < current_level->get_rows(); ++level_row) {
                for (size_t level_column = 0; level_solved && level_column < current_level->get_columns(); ++level_column) {
